...
void fractalSquarePattern(Turtle t, float distance, int recursionsLeft = 1) {
	if (recursionsLeft > 0) {
		--recursionsLeft;

		float newDistance = distance / 3;
		...
		t0.move(newDistance);
		t0.rotate(-pi / 2);
		drawSquare(t0, newDistance);

		// top-left
		Turtle t1 = t;
		t1.rotate(pi / 2);
		t1.move(2 * newDistance);
		t1.rotate(-pi / 2);
		...
		t2.move(newDistance);
		t2.rotate(pi / 2);
		t2.move(2 * newDistance);
		t2.rotate(-pi / 2);

		// top-right
		Turtle t3 = t;
		t3.move(2 * newDistance);
		t3.rotate(pi / 2);
		t3.move(2 * newDistance);
		t3.rotate(-pi / 2);
		...
		// bottom-right
		Turtle t8 = t;
		t8.move(2 * newDistance);


		fractalSquarePattern(t1, newDistance, recursionsLeft);
		fractalSquarePattern(t2, newDistance, recursionsLeft);
		fractalSquarePattern(t3, newDistance, recursionsLeft);
		fractalSquarePattern(t4, newDistance, recursionsLeft);
		fractalSquarePattern(t5, newDistance, recursionsLeft);
		fractalSquarePattern(t6, newDistance, recursionsLeft);
		fractalSquarePattern(t7, newDistance, recursionsLeft);
		fractalSquarePattern(t8, newDistance, recursionsLeft);
	}
}
...
public:
		...
		for (FloatType jj = 0, y = this->m_ymin, v = b; jj < samplePointsVertical; jj += 1, y += stepy, v += stepv) {
			for (FloatType ii = 0, x = this->m_xmin, h = l; ii < samplePointsHorizontal; ii += 1, x += stepx, h += steph) {
				...
				if (iterations == 0) {
					...
					int escapeIter = this->m_maxIteration - iterations;
					drawColor(escapeIter, red, green, blue);
	...
	void drawColor(int iterations, float& r, float& g, float& b) {
		if (iterations == 0) {
			r = 0.0f;
			g = 0.0f;
			b = 0.0f;
